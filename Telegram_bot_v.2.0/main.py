...
async def reply(user_message, chat_id):
    ...
    await assistant_func.new_thread() #create a thread
    await assistant_func.new_message(user_message) #add msg to thread
    await assistant_func.new_run() #create a new run
    await assistant_func.retrieve_run() #get the answer 
    response = await assistant_func.list_messages() #show the thread history
    ...
    try:
        while True:
            if request_list:
                ...
                tasks.append(task)
            await asyncio.sleep(1)  # Allow time for tasks to accumulate
